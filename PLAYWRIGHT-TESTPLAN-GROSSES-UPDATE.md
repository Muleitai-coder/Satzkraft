...
**Prüfstand:** Branch `agent/grosses-update-releases-1-4`, App v0.26.0 plus `Unreleased`-Mehrtag-Anzeige

**Updateumfang:** v0.23.0 O-Fix, v0.24.0 Programm-Bibliothek, v0.25.0 O-Kern, v0.26.0 Übungs-Bibliothek sowie Commit `62457d1`
...
- `BRIEFING-CODEX.md:801–844`: Produktregelwerk, Releases 1–4 und Mehrtag-Entscheidung.
- `ENTWURF-GROSSES-UPDATE.md:170–549`: freigegebene Feinspezifikationen und Abnahmekriterien.
- `CHANGELOG.md:7–36`: `Unreleased` plus v0.23.0–v0.26.0.
- `TESTBERICHT-RELEASES-1-4.md:1–75`: bisheriger Browserlauf und sechs ausdrücklich offene Automatisierungslücken.
- `ENTWURF-UEBUNGSLISTE.md`: freigegebene Datenbasis mit 200 Übungen.
- `TESTBACKUP-AUSWERTUNG.json` und `TESTPROGRAMM-ALLE-SZENARIEN.json`: bestehende realistische Testdaten.

Der vorhandene E2E-Bericht endet bei Commit `652d143` und v0.26.0. Die danach implementierte Mehrtag-Anzeige ist darin **nicht** geprüft. Reproduzierbare Playwright-Specs, WebKit-Läufe und versionierte Screenshot-Baselines fehlen bisher vollständig.
...
### Noch zu klärende Spezifikationsrisiken

| ID | Risiko | Behandlung im Testplan |
|---|---|---|
| GAP-01 | Für die neue Mehrtag-Anzeige ist kein exakter UI-Wortlaut dokumentiert. | Aktuellen Text `Letzter Wert an anderem Tag · …` vorläufig als Copy-Contract einfrieren und redaktionell abnehmen. |
| GAP-02 | Bei drei Trainingstagen fehlen dokumentierte Tie-Break- und Identitätsregeln. | Tests legen aktuelles Verhalten offen; Produktentscheidung ergänzen, bevor die Logik geändert wird. |
| GAP-03 | Carry-/Schlittenübungen sollen laut Entscheidung Gewicht **und** Zeit erfassen; Release-4-Mapping kennt nur `zeit`. | Test als `fixme`/Spezifikationsblocker führen, nicht still eine Seite annehmen. |
...
| Protokoll-Wegweiser | `Diese Einheit ist Teil deines Protokolls.` |
| Korrektur-Hinweis | `Du bearbeitest dein Protokoll. Empfehlungen späterer Wochen rechnen mit den neuen Werten.` |
| Korrekturmodus | `Du korrigierst nur die Satzwerte. Trainingszeit und Protokolleintrag bleiben unverändert.` |
...
| Absichten | `Werte korrigieren`; `Training wiederholen (ersetzt die letzte Einheit)`; `Übung nur heute tauschen`; `Ab jetzt ersetzen` |
...
| Kalibrierung | `Startgewichte finden`; `Wie finde ich mein Startgewicht?`; `Startgewicht finden` |
...
| TXT-03 | P0 | v0.22-Backup, alte abgeschlossene Einheit | Wegweiser exakt; Aktionen genau `Werte korrigieren` und `Diesen Inhalt heute trainieren`; kein Startbutton | ALL |
...
| TXT-05 | P0 | Abgeschlossene Einheit, Korrektur öffnen | Titel, Nachricht, `Weiter`, `Abbrechen` exakt | ALL |
| TXT-06 | P0 | Korrekturmodus aktiv | Inline-Hinweis exakt; Bottom-Bar ausschließlich `Fertig` | ALL |
...
| TXT-17 | P1 | Bibliotheksvorschau | `Wochenstruktur`, `Alle Übungen`, `Startgewichte finden` exakt; Warm-up/Cool-down sichtbar | FAST |
| TXT-18 | P1 | Kalibrierhinweis auf Karte und Modal | Kurzlink, Titel und vollständige Anleitung in Du-Form; keine medizinischen Aussagen | FAST |
...
| TXT-24 | P0 | Tausch vor Training | Titel, Feldlabel, Placeholder, Erklärung und Absichtsbuttons exakt | ALL |
| TXT-25 | P0 | Tausch während Training | Zusätzlich `Ab jetzt ersetzen`; Erklärung sagt, dass Editor erst nach Training bestätigt | ALL |
...
| TXT-30 | P2 | Alle Update-Ansichten | Keine Alttexte `Ordner-Symbol`, `2 · Programm laden`, `Sicher als Kopie speichern`; keine Sie-Form | DC |
...
| TXT-33 | P1 | Bereits heute getauschte Übung, Workout läuft | Vier aktuellen Absichten exakt und in stabiler Reihenfolge; Test verweist auf GAP-06 statt den vierten Weg zu verschweigen | ALL |
...
| Standardmodal | höchstens 3 Entscheidungsbuttons; Intro genau 1; der aktuelle Viererfall im Tauschmodal ist ausdrücklich GAP-06 |
...
| VIS-03 | P0 | Gesperrte alte Einheit | `protokoll-wegweiser-dunkel.png`; Erklärung und zwei Auswege klar priorisiert | ALL |
...
| VIS-08 | P1 | Lange Bibliotheksvorschau | oberer Bereich und Kalibrierabschnitt; Sticky-Aktionen überdecken keinen Text | ALL |
...
| VIS-15 | P1 | Satzpause | große Zeit, ±15 und Stop ohne Überlagerung; Scrollen weiter möglich | ALL |
...
| RED-03 | P0 | Mehrere Bar-Zustände gleichzeitig | Genau einer von Start, Fortsetzen, Wiederholen, Korrektur, Wegweiser, Workout, Pause, Hold ist sichtbar | ALL |
| RED-04 | P0 | Doppelte Protokollaktion | `Werte korrigieren`, `Diesen Inhalt heute trainieren`, Wiederholen je höchstens einmal im sichtbaren Kontext | ALL |
| RED-05 | P1 | Doppelter Bibliothekseinstieg | `#programlibrarybtn` exakt 1 und erste `.createchoice` | FAST |
| RED-06 | P1 | Zu viele Erstellwege | `.createchoice` exakt 5; IDs und Absichten eindeutig | ALL |
...
| FUN-O4-07 | P0 | Keine offene Folgewoche mehr | verständliche Fehlermeldung; keinerlei Mutation | ALL |
...
| FUN-L-04 | P1 | Vorschau komplett scrollen | alle Wochen, Deload, Tage, Übungen, WU/CD und Kalibrierung | ALL |
...
| FUN-L-12 | P1 | Startgewicht leer | Kurz- und Langhilfe erreichbar; Training erlaubt Kalibrierung | FAST |
...
| FUN-M-16 | P1 | Carry/Schlitten Gewicht+Zeit | `fixme` bis GAP-03 geklärt | — |
...
| REG-02 | P0 | Training starten → Satz → Pause → beenden | genau ein History-Eintrag, Reload erhält Zustand | ALL |
...
| REG-17 | P1 | Übung nur heute tauschen | Protokoll mit Original → Ersatz; Originalprogression unverändert | ALL |
| REG-18 | P0 | Dauerhaften Tausch vormerken | nach Workout zwingend Editor oder Verwerfen; keine Später-Option | ALL |
...
test('alte Einheit zeigt zwei klare Auswege', async ({ page }) => {
...
- Tauschtexte und deduplizierte Vorschläge;
...
- `node --test tests/*.test.cjs tests/*.test.mjs` grün ist;
