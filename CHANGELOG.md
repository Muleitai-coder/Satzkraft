...
- Beim Import wird eine im JSON vorhandene Kennung übernommen (statt neu vergeben), solange sie nicht mit einem vorhandenen Programm kollidiert. Fehlt sie, wird beim ersten Import eine vergeben. So behält ein Programm seine Identität über Export und Reimport.
- Programme tragen jetzt eine feste Kennung (`code`) und einen Stand (`revision`) im Export. Der Import erkennt daran zuverlässig, zu welchem Programm ein angepasstes JSON gehört – auch wenn eine KI den Namen geändert hat – und meldet sich direkt per Popup: „Update erkannt“ mit den Optionen Aktualisieren, als neues Programm speichern oder erst ansehen. Auch inhaltlich identische Duplikate melden sich jetzt als Popup. Wurde das Programm seit dem Export in der App geändert, warnt die Vorschau vor dem Überschreiben. Kopien, Importe und Folgeblöcke bekommen automatisch eine eigene Kennung; die KI-Anleitung erklärt, dass `code` und `revision` nie verändert werden dürfen.
