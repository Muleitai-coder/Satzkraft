# Briefing: Satzkraft – Architektur, Produktregeln und Umsetzungsstand bis v0.32.0
...
**Aktueller Produktstand:** Satzkraft v0.32.0
...
- **Auswertung (Stand 22.07.2026):** Kennzahlen-Kacheln Einheiten (mit Blockprozent), Trainingszeit und Bewegtes Gewicht untereinander in voller Breite; die früheren Fenster „Fortschritt im Trainingsblock“ und „Letzte Einheiten“ existieren nicht mehr. „Fortschritt je Übung“ fasst gleiche Übungsnamen (kanonisch, je Typ) über alle Wochentage zu einer Karte zusammen; ein Messpunkt je Ausführung, `reportSparkbars` zeigt immer 12 Plätze (letzte 12 Ausführungen, Höhen min-max-skaliert 18–100 %, offene Plätze als Stummel); getauschte Übungen und Deload-Wochen bleiben außen vor. Das Trainingsprotokoll ist eine eigene Vollbild-Seite `#protocol` (`openProtocol`/`closeProtocol`, Zurück `protback`, eigener Druckknopf `protprint`; `body.protocol-open` steuert, dass beim Drucken nur das Protokoll erscheint). Scrollbalken sind app-weit ausgeblendet (Nutzerentscheidung 22.07.2026).
- **Programmfelder:** `art` erscheint immer als eigene Zeile unter dem Programmnamen; enthält der Name das Art-Wort bereits, entfällt der Untertitel (`artSubtitleText`/`artSubtitleHtml`, Nutzerentscheidung 22.07.2026). Die `_anleitung` im Beispielprogramm dokumentiert Meta-Felder, Grenzen und erlaubte Werte auf aktuellem Stand. `art` und `bereich` sind optionale Felder im Austauschformat; `bereich` ist auf die Liste `BEREICHE` beschränkt, der Import lehnt andere Werte mit klarer Fehlermeldung ab, der Export enthält beide Felder.
